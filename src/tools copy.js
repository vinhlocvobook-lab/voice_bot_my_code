    ...
    send_message({
        ...
        item: {
            ...
            output: JSON.stringify({
                ...
                say_verbatim: "Dạ, em đang nghe ạ, mời Quý khách tiếp hoặc đọc lại mã danh bộ, 11 chữ số, đọc liên tục, liền mạch, không ngắt quãng giúp em ạ !"
            }),
        }
    })
    send_message({
...
export async function get_bill_handler_old1(function_event, asteriskData, send_message) {
    ...
    let kq_get_so_danh_bo = null;
    try {
        kq_get_so_danh_bo = await get_so_danh_bo(asteriskData);
    } catch (error) {
        console.log("\n\r \n\r\n \r [tools][get_bill_handler] error: ", error);
        if (error.message === "old_tool") {
            log.warn("[tools][get_bill_handler] old_tool");
    ...
    console.log("\n\r \n\r");
    const { ma_danh_bo: mdb_gpt, xac_nhan, ma_danh_bo_length, do_tin_cay, ly_do } = kq_get_so_danh_bo || {}
    ...
    try {
        if (mdb_gpt == "" || mdb_gpt == null || mdb_gpt == "undefined") {
            ...
            // nếu chưa có mã danh bộ thì cần nhờ khách hàng đọc mã danh bộ
            if (asteriskData.ma_danh_bo_count == 0) {
                asteriskData.ma_danh_bo_count++;
                sdb_doc_mdb_chua_co(function_event, send_message)
                return;
            } else {
                if (ma_danh_bo_length < 11) {
                    sdb_doc_mdb_chua_du(function_event, send_message);
                    asteriskData.ma_danh_bo_count++;
...
export async function get_so_danh_bo_handler(function_event, asteriskData, send_message) {
...
export function tool_function_handler(function_event, asteriskData, send_message) {
    ...
    } else if (function_event.name == "get_so_danh_bo") {
        return get_so_danh_bo_handler(function_event, asteriskData, send_message);
    } else {
